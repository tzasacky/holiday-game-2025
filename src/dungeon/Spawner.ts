...
export class Spawner {
    ...
    static spawnMobs(level: Level, config: SpawnConfig) {
        ...
        // Use room centers for spawning, but skip the first room (entrance)
        const availableSpawns = level.rooms.slice(1).map(room => room.center);
    ...
    private static isValidSpawnPosition(level: Level, pos: ex.Vector): boolean {
        ...
        if (!level.isWalkable(pos.x, pos.y)) {
            Logger.debug(`[Spawner] Position ${pos} not walkable`);
            return false;
        }
        ...
        if (player && player.gridPos.equals(pos)) {
            Logger.warn(`[Spawner] Position ${pos} occupied by player at ${player.gridPos}`);
            return false;
        }
        
        Logger.debug(`[Spawner] Position ${pos} is valid (${level.actors.length} actors in level)`);
        return true;
    }
}
