...
/**
 * Batch Tileset Processor - Processes all generated tilesets according to sprites.yaml
 * Usage: node tools/process_tilesets.js
 */

const { processSingleTileset } = require('./tileset_processor.js');
const fs = require('fs');
...
async function main() {
    console.log('🧱 Holiday Game Tileset Processor');
    console.log('=================================');
    ...
    for (const entry of spriteConfig) {
        ...
        console.log(`   Grid: ${spec.cols}x${spec.rows} (32x32 tiles)`);
        ...
        try {
            ...
            const options = {
                tolerance: 20, // Low tolerance for tilesets
                verbose: verbose
            };
            
            await processSingleTileset(
                inputFile,
                outputFile,
                spec.cols,
                spec.rows,
                32, // 32x32 target tile size
                options
            );
    ...
    console.log('\n🎯 Next Steps:');
    console.log('1. Check the processed tilesets in public/assets/');
    console.log('2. Test the tilesets in your game');
}
